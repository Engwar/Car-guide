...
class CarsTableViewController: UITableViewController {
	...
	override func tableView(_ tableView: UITableView, moveRowAt fromIndexPath: IndexPath, to toThe: IndexPath) {
		let removeCar = cars.remove(at: fromIndexPath.row)
		cars.insert(removeCar, at: toThe.row)
	...
	@IBAction func unwind(segue: UIStoryboardSegue) {
		...
		let vContr = segue.source as? AddEditCarsTableViewController
		let car = vContr?.car
		if let selectedPath = tableView.indexPathForSelectedRow {
			cars[selectedPath.row] = car ?? car.unsafelyUnwrapped
			tableView.reloadRows(at: [selectedPath], with: .automatic)
		} else {
			let indexPath = IndexPath(row: cars.count, section: 0)
			cars.append(car ?? car.unsafelyUnwrapped)
